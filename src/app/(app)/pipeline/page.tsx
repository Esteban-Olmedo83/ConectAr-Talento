...
function EmailModal({
  ...
  const gmailHref = `https://mail.google.com/mail/?view=cm&fs=1&to=${encodeURIComponent(candidate.email)}${previewSubject || previewBody ? '&' : ''}${previewSubject ? `su=${encodeURIComponent(previewSubject)}` : ''}${previewSubject && previewBody ? '&' : ''}${previewBody ? `body=${encodeURIComponent(previewBody)}` : ''}`
          ...
          return `
            <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:10px;padding:16px;margin-bottom:14px;">
              <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:8px;">
                <div>
                  <p style="font-size:15px;font-weight:700;color:#111827;margin:0;">${escapeHtml(vac?.title) || '—'}</p>
                  ${vac?.client?.name ? `<p style="font-size:12px;color:#6b7280;margin:2px 0 0;">${escapeHtml(vac.client.name)}</p>` : ''}
                </div>
