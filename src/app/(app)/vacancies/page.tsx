...
const PRIORITY_CONFIG: Record<VacancyPriority, { label: string; cls: string }> = {
  Alta: { label: 'Alta', cls: 'bg-red-500/15 text-red-300 border-red-500/25' },
  Media: { label: 'Media', cls: 'bg-amber-500/15 text-amber-300 border-amber-500/25' },
  Baja: { label: 'Baja', cls: 'bg-gray-500/15 text-gray-400 border-gray-500/25' },
}
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          ...
          <div className="grid grid-cols-2 gap-3 p-3 bg-indigo-500/10 rounded-lg border border-indigo-500/20">
            <div>
              <label className={cn(labelCls, 'text-indigo-300')}>Rubro</label>
  ...
  return (
    <Card className="hover:shadow-md transition-shadow group relative">
      <CardContent className="p-4">
        ...
        <div className="text-xs font-semibold text-emerald-400 mb-3">{salaryStr}</div>

        {/* Skills */}
        {vacancy.requirements.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-3">
            {vacancy.requirements.slice(0, 3).map(s => (
              <span key={s} className="text-[10px] px-1.5 py-0.5 bg-indigo-500/15 text-indigo-300 rounded-full">{s}</span>
            ))}
